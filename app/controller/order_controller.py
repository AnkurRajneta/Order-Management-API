...
@router.post("")
async def create_order_controller(payload:CreateOrder, db:AsyncSession = Depends(get_db)):
...
async def get_all_order_controller(db:AsyncSession = Depends(get_db)):
    service = OrderService(db)
    result = await service.get_all_order_service()
    return result

@router.get("/id/{id}", response_model=OrderOut)
...
async def update_order_by_id(id:int, payload:UpdateOrder, db:AsyncSession = Depends(get_db)):
    service = OrderService(db)
